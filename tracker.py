...
# Estado en memoria para posiciones de wallets trackeadas
wallet_positions = {}
ws_app = None
ws_thread = None
...
def on_ws_message(ws, message):
    try:
        ...
        new_assets = {p["asset"] for p in new_positions}
        # Detectar posiciones nuevas
        for pos in new_positions:
            if pos["asset"] not in old_assets:
                print(f"[WS] Nueva posición detectada en {w['label']}: {pos['side']} {pos['asset']}")
...
def on_ws_open(ws):
    ...
    for w in wallets:
        ...
        print(f"[WS] Suscrito a {w['label']} ({w['address'][:10]}...)")
...
def init_websocket():
    global ws_thread
    # Cargar posiciones iniciales via REST
    wallets = load_wallets()
    for w in wallets:
        get_wallet_stats(w["address"])
    # Arrancar WebSocket en thread separado
